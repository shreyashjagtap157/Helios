...
function startLanguageServer(context: vscode.ExtensionContext) {
    ...
    if (!config.get<boolean>('lsp.enabled', true)) {
        outputChannel.appendLine('Omni language server is disabled via settings.');
        return;
    }
    ...
    if (!lspPath) {
        ...
        if (workspaceRoot) {
            const candidates = [
                path.join(workspaceRoot, 'tools', 'omni-lsp', 'target', 'release', 'omni-lsp' + ext),
                path.join(workspaceRoot, 'tools', 'omni-lsp', 'target', 'debug', 'omni-lsp' + ext),
            ];
        ...
        if (!lspPath) {
            const extRelative = path.join(context.extensionPath, '..', '..', 'tools', 'omni-lsp', 'target', 'debug', 'omni-lsp' + ext);
            try {
                fs.accessSync(extRelative, fs.constants.X_OK);
                lspPath = extRelative;
            } catch {
                // Last resort: try PATH
                lspPath = 'omni-lsp' + ext;
            }
        }
    }
