...
impl UnsafeAuditor {
    ...
    fn audit_file(path: &std::path::Path, reports: &mut Vec<UnsafeReport>) {
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            if trimmed.contains("unsafe {") || trimmed.contains("unsafe:") || trimmed.starts_with("unsafe ") {
                reports.push(UnsafeReport {
                    file: file_str.clone(),
                    line: line_num + 1,
                    reason: format!("Unsafe block: {}", trimmed.chars().take(80).collect::<String>()),
                });
            }
            ...
            if trimmed.contains("*mut ") || trimmed.contains("*const ") {
                reports.push(UnsafeReport {
                    file: file_str.clone(),
                    line: line_num + 1,
                    reason: format!("Raw pointer usage: {}", trimmed.chars().take(80).collect::<String>()),
                });
            }
            ...
            if trimmed.contains("extern \"C\"") || trimmed.contains("extern \"system\"") {
                reports.push(UnsafeReport {
                    file: file_str.clone(),
                    line: line_num + 1,
                    reason: format!("FFI boundary: {}", trimmed.chars().take(80).collect::<String>()),
                });
            }
        }
    }
