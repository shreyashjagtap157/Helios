...
impl SafetyChecker {
    ...
    pub fn run_tsan(binary_path: &str) -> Result<String, String> {
        ...
        let output = Command::new(binary_path)
            .env("TSAN_OPTIONS", "halt_on_error=1:second_deadlock_stack=1:detect_deadlocks=1")
            .output()
    ...
    pub fn check_leaks() -> Vec<LeakReport> {
        ...
        #[cfg(target_os = "linux")]
        {
            if let Ok(maps) = std::fs::read_to_string("/proc/self/maps") {
                let anon_regions: Vec<&str> = maps.lines()
                ...
                if anon_regions.len() > 100 {
                    leaks.push(LeakReport {
                        ...
                        detail: format!("{} anonymous memory regions (possible fragmentation)", anon_regions.len()),
                    });
                }
            }
        }
    ...
    pub fn visualize_lifetimes(func_name: &str, 
        ...
        for (name, birth, death) in variables {
            ...
            let bar: String = std::iter::repeat('█').take(lifetime_pct.max(1).min(50)).collect();
            dot.push_str(&format!(
                "  {} [label=\"{{ {} | L{}-L{} | {} }}\"];\n",
                sanitize_dot_id(name), name, birth, death, bar
            ));
        }
        ...
        for (borrower, lender, kind) in borrows {
            ...
            dot.push_str(&format!(
                "  {} -> {} [label=\"{}\", color=\"{}\", style=\"{}\"];\n",
                sanitize_dot_id(lender), sanitize_dot_id(borrower), label, color, style
            ));
        }
        ...
        for (i, (b1, l1, k1)) in borrows.iter().enumerate() {
            for (b2, l2, k2) in borrows.iter().skip(i + 1) {
                if l1 == l2 && matches!(k1, BorrowKind::MutableRef) && matches!(k2, BorrowKind::MutableRef) {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        for line in text.lines() {
            if line.starts_with("WorkingSetSize=") {
                let bytes: usize = line.split('=').nth(1).and_then(|s| s.trim().parse().ok()).unwrap_or(0);
