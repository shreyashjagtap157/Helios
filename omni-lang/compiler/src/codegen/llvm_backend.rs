...
use crate::ir::{IrModule, IrFunction, IrType, IrInstruction, IrTerminator, IrBlock};
use inkwell::context::Context;
use inkwell::builder::Builder;
use inkwell::module::Module;
use inkwell::targets::{Target, TargetMachine, RelocMode, CodeModel, FileType, InitializationConfig};
use inkwell::types::{BasicType, BasicTypeEnum, FunctionType, StructType, BasicMetadataTypeEnum};
...
impl<'ctx> LlvmCodegen<'ctx> {
    ...
    fn declare_runtime_functions(&mut self) {
        ...
        self.module.add_function("__omni_gc_alloc", alloc_fn_type, None);
        
        // GC safepoint
        let safepoint_fn_type = void_type.fn_type(&[], false);
        self.module.add_function("__omni_check_safepoint", safepoint_fn_type, None);
        
        // Panic handler
        let panic_fn_type = void_type.fn_type(&[i8_ptr.into()], false);
        self.module.add_function("__omni_panic", panic_fn_type, None);
        
        // Bound check
        let bound_check_type = void_type.fn_type(&[i64_type.into(), i64_type.into()], false);
        self.module.add_function("__omni_bounds_check", bound_check_type, None);
        
        // Debug print
        let print_fn_type = void_type.fn_type(&[i8_ptr.into()], false);
        self.module.add_function("__omni_debug_print", print_fn_type, None);
    }
    ...
    fn generate_function(&mut self, ir_func: &IrFunction) -> Result<(), String> {
        ...
        let function = *self.functions.get(&ir_func.name)
        ...
        let entry_block = self.current_blocks.get(&ir_func.blocks[0].label)
    ...
    fn generate_block(&mut self, block: &IrBlock) -> Result<(), String> {
        let bb = *self.current_blocks.get(&block.label)
    ...
    fn generate_instruction(&mut self, inst: &IrInstruction) -> Result<(), String> {
        match inst {
            IrInstruction::Alloca { dest, ty } => {
                let llvm_type = self.ir_type_to_llvm(ty)?;
                let alloca = self.builder.build_alloca(llvm_type, dest)
            ...
            IrInstruction::Load { dest, ptr, ty } => {
                ...
                let llvm_type = self.ir_type_to_llvm(ty)?;
                let loaded = self.builder.build_load(llvm_type, ptr_val, dest)
                    .map_err(|e| format!("Load failed: {:?}", e))?;
                
                // Store in a temporary local
                let temp = self.builder.build_alloca(llvm_type, &format!("{}_temp", dest))
            ...
            IrInstruction::BinOp { dest, op, left, right } => {
                ...
                let temp = self.builder.build_alloca(ty, dest)
                    .map_err(|e| format!("Alloca for binop failed: {:?}", e))?;
                self.builder.build_store(temp, result)
            ...
            IrInstruction::Call { dest, func, args } => {
                ...
                let call_site = self.builder.build_call(*callee, &llvm_args, "call")
                    .map_err(|e| format!("Call failed: {:?}", e))?;
                
                if let Some(dest_name) = dest {
                    if let Some(ret_val) = call_site.try_as_basic_value().left() {
                        let temp = self.builder.build_alloca(ret_val.get_type(), dest_name)
                            .map_err(|e| format!("Alloca for call result failed: {:?}", e))?;
                        self.builder.build_store(temp, ret_val)
            ...
            IrInstruction::GetField { dest, ptr, field } => {
                ...
                let pointee_type: BasicTypeEnum<'ctx> = self.types.iter()
                    .find(|(name, _)| ptr.starts_with(name.as_str()) || ptr.ends_with(name.as_str()))
                    .map(|(_, st)| (*st).into())
                    .unwrap_or_else(|| {
                ...
                let gep = unsafe {
                    self.builder.build_gep(
                        pointee_type,
                        ptr_val,
                        &[i32_type.const_int(0, false), i32_type.const_int(*field as u64, false)],
                        dest,
    ...
    fn generate_int_binop(
        ...
        let result = match op {
            ...
            "eq" => self.builder.build_int_compare(IntPredicate::EQ, lhs, rhs, name),
            "ne" => self.builder.build_int_compare(IntPredicate::NE, lhs, rhs, name),
            "lt" | "slt" => self.builder.build_int_compare(IntPredicate::SLT, lhs, rhs, name),
            "le" | "sle" => self.builder.build_int_compare(IntPredicate::SLE, lhs, rhs, name),
            "gt" | "sgt" => self.builder.build_int_compare(IntPredicate::SGT, lhs, rhs, name),
            "ge" | "sge" => self.builder.build_int_compare(IntPredicate::SGE, lhs, rhs, name),
            "ult" => self.builder.build_int_compare(IntPredicate::ULT, lhs, rhs, name),
            "ule" => self.builder.build_int_compare(IntPredicate::ULE, lhs, rhs, name),
            "ugt" => self.builder.build_int_compare(IntPredicate::UGT, lhs, rhs, name),
            "uge" => self.builder.build_int_compare(IntPredicate::UGE, lhs, rhs, name),
    ...
    fn generate_float_binop(
        ...
        let result = match op {
            ...
            "feq" | "oeq" => self.builder.build_float_compare(FloatPredicate::OEQ, lhs, rhs, name),
            "fne" | "one" => self.builder.build_float_compare(FloatPredicate::ONE, lhs, rhs, name),
            "flt" | "olt" => self.builder.build_float_compare(FloatPredicate::OLT, lhs, rhs, name),
            "fle" | "ole" => self.builder.build_float_compare(FloatPredicate::OLE, lhs, rhs, name),
            "fgt" | "ogt" => self.builder.build_float_compare(FloatPredicate::OGT, lhs, rhs, name),
            "fge" | "oge" => self.builder.build_float_compare(FloatPredicate::OGE, lhs, rhs, name),
    ...
    fn generate_terminator(&mut self, term: &IrTerminator) -> Result<(), String> {
        match term {
            ...
            IrTerminator::Branch(label) => {
                let target = *self.current_blocks.get(label)
            ...
            IrTerminator::CondBranch { cond, then_label, else_label } => {
                ...
                let then_block = *self.current_blocks.get(then_label)
                    .ok_or_else(|| format!("Unknown then block: {}", then_label))?;
                let else_block = *self.current_blocks.get(else_label)
    ...
    fn ir_type_to_llvm(&self, ty: &IrType) -> Result<BasicTypeEnum<'ctx>, String> {
        match ty {
            ...
            IrType::Ptr(_) => Ok(self.context.i8_type().ptr_type(AddressSpace::default()).into()),
