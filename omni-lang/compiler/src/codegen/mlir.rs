...
use crate::ir::{IrModule, IrInstruction, IrValue};
...
impl Default for TileSizes {
    fn default() -> Self {
        ...
        Self { l1_tile: 64, l2_tile: 256, l3_tile: 1024 }
    }
}
...
pub enum LinalgOp {
    ...
    Conv2D {
        batch: usize, height: usize, width: usize, channels: usize,
        kernel_h: usize, kernel_w: usize, filters: usize,
        input: String, kernel: String, output: String,
    },
    BatchMatMul {
        batch: usize, m: usize, n: usize, k: usize,
        a: String, b: String, c: String,
    },
    Elementwise { op: String, inputs: Vec<String>, output: String },
    Reduce { op: String, input: String, output: String, axis: usize },
    Generic,
}
...
pub enum AffineOp {
    For(AffineFor),
    Load { base: String, indices: Vec<AffineExpr> },
    Store { base: String, value: String, indices: Vec<AffineExpr> },
    Compute { op: String, lhs: String, rhs: String, dest: String },
    Prefetch { base: String, indices: Vec<AffineExpr>, locality: u8 },
}
...
#[derive(Debug, Clone)]
pub enum TensorOp {
    MatMul { lhs: String, rhs: String, out: String, m: usize, n: usize, k: usize },
    Conv2D { input: String, kernel: String, out: String },
    Relu { input: String, out: String },
    Softmax { input: String, out: String },
    Transpose { input: String, out: String, perm: Vec<usize> },
}
...
#[derive(Debug, Clone)]
pub enum VectorOp {
    Load { base: String, index: AffineExpr, width: usize },
    Store { base: String, index: AffineExpr, width: usize, value: String },
    Fma { a: String, b: String, c: String, width: usize },
    Broadcast { scalar: String, width: usize },
    Reduce { op: String, vector: String, width: usize },
}
...
pub fn lower_ast_to_mlir(ctx: &MlirContext, module: &IrModule) -> Result<Vec<OmniDialect>, String> {
    log::info!("Lowering module '{}' to MLIR... ({} functions)", module.name, module.functions.len());
    ...
    for func in &module.functions {
        for block in &func.blocks {
            for inst in &block.instructions {
                if let IrInstruction::Call { dest, func: callee, args } = inst {
                    match callee.as_str() {
                        "matmul" | "torch.mm" | "np.dot" | "__omni_matmul" => {
                            ...
                            ops.push(TensorOp::MatMul { 
                                ...
                                m: 0, n: 0, k: 0,  // Shapes resolved during linalg lowering
                            });
                        },
...
fn extract_binary_args(args: &[IrValue]) -> (String, String) {
    let a = args.get(0).map(|v| format!("{:?}", v)).unwrap_or_else(|| "%a".into());
    let b = args.get(1).map(|v| format!("{:?}", v)).unwrap_or_else(|| "%b".into());
    (a, b)
}

fn extract_unary_arg(args: &[IrValue]) -> String {
    args.get(0).map(|v| format!("{:?}", v)).unwrap_or_else(|| "%x".into())
}

fn lower_to_linalg(ops: Vec<TensorOp>) -> Vec<LinalgOp> {
    ops.into_iter().map(|op| match op {
        TensorOp::MatMul { lhs, rhs, out, m, n, k } => {
            ...
            LinalgOp::MatMul { m, n, k, a: lhs, b: rhs, c: out }
        },
        TensorOp::Conv2D { input, kernel, out } => LinalgOp::Conv2D {
            batch: 1, height: 224, width: 224, channels: 3,
            kernel_h: 3, kernel_w: 3, filters: 64,
            input, kernel, output: out,
        },
        TensorOp::Relu { input, out } => LinalgOp::Elementwise {
            op: "max(0, x)".into(), inputs: vec![input], output: out,
        },
        TensorOp::Softmax { input, out } => LinalgOp::Reduce {
            op: "softmax".into(), input, output: out, axis: 0,
        },
        _ => LinalgOp::Generic,
    }).collect()
}
    ...
    for op in ops {
        match op {
            LinalgOp::MatMul { m, n, k, a, b, c } => {
                log::info!("Polyhedral tiling MatMul [{}×{} * {}×{}] with tiles L1={}, L2={}", 
                    m, k, k, n, tiles.l1_tile, tiles.l2_tile);
                ...
                let tiled_loop = AffineOp::For(AffineFor {
                    iter: "ii".into(), start: 0, end: m as i64, step: l2,
                    body: vec![AffineOp::For(AffineFor {
                        iter: "jj".into(), start: 0, end: n as i64, step: l2,
                        body: vec![AffineOp::For(AffineFor {
                            iter: "kk".into(), start: 0, end: k as i64, step: l2,
                            body: vec![
                                // L1 tile
                                AffineOp::For(AffineFor {
                                    iter: "i".into(), start: 0, end: l2, step: l1,
                                    body: vec![AffineOp::For(AffineFor {
                                        iter: "j".into(), start: 0, end: l2, step: l1,
                                        body: vec![AffineOp::For(AffineFor {
                                            iter: "k".into(), start: 0, end: l2, step: 1,
                                            body: vec![
            ...
            LinalgOp::Conv2D { batch, height, width, channels, kernel_h, kernel_w, filters, input, kernel, output } => {
                log::info!("Polyhedral tiling Conv2D [{}×{}×{}×{}, {}×{}→{}]",
                    batch, height, width, channels, kernel_h, kernel_w, filters);
                ...
                let conv_loop = AffineOp::For(AffineFor {
                    iter: "n".into(), start: 0, end: batch as i64, step: 1,
                    body: vec![AffineOp::For(AffineFor {
                        iter: "oh".into(), start: 0, end: (height - kernel_h + 1) as i64, step: tile_h,
                        body: vec![AffineOp::For(AffineFor {
                            iter: "ow".into(), start: 0, end: (width - kernel_w + 1) as i64, step: tile_w,
                            body: vec![AffineOp::For(AffineFor {
                                iter: "f".into(), start: 0, end: filters as i64, step: 1,
                                body: vec![AffineOp::For(AffineFor {
                                    iter: "kh".into(), start: 0, end: kernel_h as i64, step: 1,
                                    body: vec![AffineOp::For(AffineFor {
                                        iter: "kw".into(), start: 0, end: kernel_w as i64, step: 1,
                                        body: vec![AffineOp::For(AffineFor {
                                            iter: "c".into(), start: 0, end: channels as i64, step: 1,
                                            body: vec![
                                                AffineOp::Compute {
                                                    op: "fma".into(),
            ...
            LinalgOp::Elementwise { op, inputs, output } => {
                ...
                let ewise_loop = AffineOp::For(AffineFor {
                    iter: "i".into(), start: 0, end: -1, step: 1, // -1 = dynamic size
            ...
            LinalgOp::Reduce { op, input, output, axis: _axis } => {
                // Reduction: 2-level loop, outer over non-reduced dims, inner accumulates
                let reduce_loop = AffineOp::For(AffineFor {
                    iter: "i".into(), start: 0, end: -1, step: 1,
                    body: vec![AffineOp::For(AffineFor {
                        iter: "j".into(), start: 0, end: -1, step: 1,
                        body: vec![AffineOp::Compute {
...
fn vectorize_affine_op(op: &AffineOp, vector_width: usize) -> OmniDialect {
    match op {
        AffineOp::For(af) => {
            ...
            let is_innermost = af.body.iter().all(|op| matches!(op, AffineOp::Compute { .. } | AffineOp::Prefetch { .. }));
            ...
            if is_innermost && af.step == 1 {
                ...
                for body_op in &af.body {
                    if let AffineOp::Compute { op: _compute_op, lhs, rhs, dest } = body_op {
                ...
                OmniDialect::Affine(AffineOp::For(AffineFor {
                    ...
                    body: vec_ops.into_iter().map(|v| {
                        if let OmniDialect::Vector(vop) = v {
                            ...
                            AffineOp::Compute { op: "nop".into(), lhs: String::new(), rhs: String::new(), dest: String::new() }
                        }
                    }).collect(),
                }))
            } else {
                // Recurse into nested loops
                let new_body: Vec<AffineOp> = af.body.iter().map(|inner| {
                    if let OmniDialect::Affine(inner_af) = vectorize_affine_op(inner, vector_width) {
                        inner_af
                    } else {
                        inner.clone()
                    }
...
fn format_op(op: &OmniDialect, indent: usize) -> String {
    ...
    match op {
        OmniDialect::Affine(AffineOp::For(af)) => {
            let mut s = format!("{}affine.for %{} = {} to {} step {} {{\n", 
                pad, af.iter, af.start, af.end, af.step);
            for body_op in &af.body {
                s.push_str(&format_op(&OmniDialect::Affine(body_op.clone()), indent + 1));
            }
            s.push_str(&format!("{}}}\n", pad));
            s
        },
        ...
        OmniDialect::Affine(AffineOp::Prefetch { base, indices, locality }) => {
            let idx_str: Vec<String> = indices.iter().map(|i| format!("{}", i)).collect();
            format!("{}affine.prefetch {}[{}], locality={}\n", pad, base, idx_str.join(", "), locality)
        },
