...
use crate::ir::{IrBlock, IrFunction, IrInstruction, IrModule, IrValue};
...
impl KernelFusionPass {
    pub fn run(module: &mut IrModule) {
        ...
        log::info!(
            "Kernel Fusion: merged {} operation chains into fused kernels",
            total_fused
        );
    }
    ...
    fn fuse_kernels(func: &mut IrFunction) -> usize {
        ...
        for block_idx in 0..func.blocks.len() {
            ...
            log::info!(
                "Found {} fusion chains in {}::{}",
                chains.len(),
                func.name,
                func.blocks[block_idx].label
            );
    ...
    fn maybe_save_chain(
        ...
        if indices.len() >= 2 {
            ...
            let chain_dests: Vec<String> = indices
                .iter()
    ...
    fn apply_fusion(block: &mut IrBlock, chain: &FusionChain) {
        let fused_name = format!("__fused_kernel_{}", chain.op_names.join("_"));

        // Build args from external inputs
        let args: Vec<IrValue> = chain
            .external_inputs
            .iter()
