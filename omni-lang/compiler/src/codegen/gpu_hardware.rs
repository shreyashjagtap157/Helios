...
impl PtxAssembler {
    ...
    pub fn assemble(&self, ptx_text: &str, output_path: &Path) -> Result<Vec<u8>, String> {
        ...
        fs::write(ptx_file, ptx_text).map_err(|e| format!("Failed to write PTX file: {}", e))?;
        ...
        fs::read(output_path).map_err(|e| format!("Failed to read assembled binary: {}", e))
    }
...
impl CudaRuntime {
    ...
    pub fn new(ptx_binary: &[u8], device_id: i32) -> Result<Self, String> {
        ...
        Ok(CudaRuntime { device_id, module })
    }
...
impl VulkanRuntime {
    ...
    pub fn allocate_buffer(&self, size: usize) -> Result<VulkanBuffer, String> {
        ...
        Ok(VulkanBuffer { handle: 0x0, size })
    }

    pub fn copy_to_buffer(&self, data: &[u8], buffer: &mut VulkanBuffer) -> Result<(), String> {
...
impl GpuExecutionProfile {
    ...
    pub fn compute_efficiency(&self) -> f64 {
        let total_ms =
            self.compilation_time_ms + self.kernel_execution_time_ms + self.memory_transfer_time_ms;
        if total_ms == 0 {
            0.0
        } else {
