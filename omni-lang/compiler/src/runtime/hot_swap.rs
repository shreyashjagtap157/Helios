...
impl HotFunction {
    ...
    pub fn patch(&self, new_ptr: usize) {
        info!("Hot Reload: Patching function '{}' (version {})", 
            self.name, self.version.load(Ordering::Relaxed));
        ...
        info!("Hot Reload: Function '{}' patched to version {}", 
            self.name, self.version.load(Ordering::Relaxed));
    }
...
impl HotSwapManager {
    ...
    pub fn register(&mut self, name: &str, ptr: usize) {
        ...
        self.registry.insert(name.to_string(), HotFunction::new(name, ptr));
    }
    ...
    pub fn update(&self, name: &str, new_ptr: usize) -> Result<(), String> {
        if let Some(func) = self.registry.get(name) {
            ...
            Err(format!("Function '{}' not found in hot swap registry", name))
        }
    }
    ...
    pub fn check_for_updates(&mut self) -> bool {
        ...
        if !changes.is_empty() {
            ...
            for change in &changes {
                ...
                self.watched_files.insert(change.path.clone(), change.modified_at);
    ...
    pub fn apply_pending(&mut self) {
        ...
        for change in &changes {
            ...
            debug!("Hot Swap: Would recompile and patch functions from {:?}", change.path);
        }
    }
    
    /// Get a function's current version
    pub fn version(&self, name: &str) -> Option<usize> {
        self.registry.get(name).map(|f| f.version.load(Ordering::Relaxed))
    }
