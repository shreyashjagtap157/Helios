...
impl NetworkManager {
    ...
    pub fn tcp_accept(&mut self, listener_handle: usize) -> Result<usize, String> {
        ...
        self.next_handle += 1;
        debug!(
            "Network: Accepted connection from {} (handle={})",
            addr, handle
        );
        self.tcp_streams.insert(handle, stream);
        Ok(handle)
    ...
    pub fn udp_bind(&mut self, addr: &str) -> Result<usize, String> {
        let socket = UdpSocket::bind(addr).map_err(|e| format!("UDP bind failed: {}", e))?;
        let handle = self.next_handle;
        self.next_handle += 1;
    ...
    pub fn http_get(&mut self, url: &str) -> Result<String, String> {
        ...
        let mut stream =
            TcpStream::connect(&addr).map_err(|e| format!("HTTP connect failed: {}", e))?;
