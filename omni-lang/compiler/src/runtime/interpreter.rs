...
impl Interpreter {
    ...
    pub fn eval_file(&mut self, path: &std::path::Path) -> Result<RuntimeValue> {
        ...
        if !path.exists() {
            return Err(anyhow!("Error[E001]: source file '{}' does not exist", path.display()));
        }
        ...
        let source = std::fs::read_to_string(path)
            .map_err(|e| anyhow!("Error[E002]: failed to read source file '{}': {}", path.display(), e))?;
    ...
    pub fn eval_source(&mut self, source: &str, filename: &str) -> Result<RuntimeValue> {
        ...
        let tokens = tokenize(source)
            .map_err(|e| anyhow!("Error[E010] in {}: {}", filename, e))?;
    ...
    pub fn eval_statement(&mut self, stmt: &crate::parser::ast::Statement) -> Result<RuntimeValue> {
        ...
        match stmt {
            Statement::Let { name, mutable: _, ty: _, value } => {
            ...
            Statement::For { var, iter, body } => {
                ...
                match iterable {
                    ...
                    RuntimeValue::Integer(end) => {
                        // Range 0..end
                        for i in 0..end {
                            self.global_scope.borrow_mut().set(var.clone(), RuntimeValue::Integer(i));
                            let result = self.eval_block(body);
                            match &result {
                                Err(_e) if format!("{}", _e) == "__break__" => break,
                                Err(_e) if format!("{}", _e) == "__continue__" => continue,
                ...
                match target {
                    Expression::Identifier(name) => {
                        let final_val = if let Some(binop) = op {
                            let current = self.global_scope.borrow().get(name)
                    ...
                    Expression::Field(obj_expr, field_name) => {
                        ...
                        if let Expression::Identifier(var_name) = obj_expr.as_ref() {
                            let current_obj = self.global_scope.borrow().get(var_name)
                                .ok_or_else(|| anyhow!("Undefined variable: {}", var_name))?;
                            match current_obj {
                                RuntimeValue::StructInstance { type_name, mut fields } => {
                                    let final_val = if let Some(binop) = op {
                                        let current = fields.get(field_name)
                                            .ok_or_else(|| anyhow!("No field '{}' on struct", field_name))?
                                            .clone();
                                        self.apply_binary_op(binop, current, new_val)?
                                    } else {
                                        new_val
                                ...
                                RuntimeValue::Map(mut m) => {
                                    let final_val = if let Some(binop) = op {
                                        let current = m.get(field_name)
                                            .ok_or_else(|| anyhow!("No field '{}' on map", field_name))?
                                            .clone();
                                        self.apply_binary_op(binop, current, new_val)?
                                    } else {
                                        new_val
                                ...
                                _ => return Err(anyhow!("Cannot set field '{}' on {:?}", field_name, current_obj)),
                            }
                        }
                    }
            ...
            Statement::Match { expr, arms } => {
                ...
                for arm in arms {
                    if let Some(bindings) = self.match_pattern(&arm.pattern, &subject) {
                        ...
                        for (name, value) in &bindings {
                            self.global_scope.borrow_mut().set(name.clone(), value.clone());
                        }
                        // Evaluate body
                ...
                Err(anyhow!("Match exhausted: no arm matched value {:?}", subject))
            }
            _ => Ok(RuntimeValue::Null), // Other statements handled minimally
        }
    }

    fn match_pattern(&self, pattern: &crate::parser::ast::Pattern, value: &RuntimeValue) -> Option<std::collections::HashMap<String, RuntimeValue>> {
        use crate::parser::ast::{Pattern, Literal};
        ...
        match pattern {
            ...
            Pattern::Constructor(name, _sub_patterns) => {
                // Check if value is a struct instance with matching type name
                match value {
                    RuntimeValue::StructInstance { type_name, fields: _ } => {
                        if type_name != name { return None; }
                        Some(bindings)
                    }
                    _ => None,
                }
    ...
    pub fn eval_expr(&mut self, expr: &crate::parser::ast::Expression) -> Result<RuntimeValue> {
        ...
        match expr {
            ...
            Expression::Identifier(name) => {
                ...
                match name.as_str() {
                    "print" | "println" | "len" | "assert" | "type_of" | "format" | "str" | "int" | "float" | "range" | "input" => {
            ...
            Expression::Call(callee, args) => {
                ...
                match func_val {
                    RuntimeValue::Function(func) => {
                        self.call_function_ast(&*func, &eval_args)
                    }
                    RuntimeValue::NativeFunction(name) => {
            ...
            Expression::Field(obj, field) => {
                ...
                match val {
                    RuntimeValue::Map(m) => {
                        m.get(field).cloned().ok_or_else(|| anyhow!("No field '{}' on map", field))
                    }
                    RuntimeValue::StructInstance { ref fields, .. } => {
                        fields.get(field).cloned().ok_or_else(|| anyhow!("No field '{}' on struct", field))
                    }
            ...
            Expression::Index(obj, idx) => {
                ...
                match (&val, &index) {
                    ...
                    (RuntimeValue::List(a), RuntimeValue::Integer(i)) => {
                        ...
                        a.get(idx).cloned().ok_or_else(|| anyhow!("Index {} out of bounds (len {})", i, a.len()))
                    }
                    (RuntimeValue::Map(m), RuntimeValue::String(k)) => {
                        m.get(k).cloned().ok_or_else(|| anyhow!("Key '{}' not found", k))
                    }
            ...
            Expression::Lambda { params, body } => {
                let func = crate::parser::ast::Function {
                    ...
                    body: crate::parser::ast::Block {
                        statements: vec![crate::parser::ast::Statement::Return(Some(*body.clone()))],
                    },
                };
            ...
            _ => Err(anyhow!("Expression type not yet supported: {:?}", std::mem::discriminant(expr))),
        }
    }

    /// Call an AST function with given arguments
    fn call_function_ast(&mut self, func: &crate::parser::ast::Function, args: &[RuntimeValue]) -> Result<RuntimeValue> {
    ...
    fn call_function_with_self(&mut self, func: &crate::parser::ast::Function, self_val: RuntimeValue, args: &[RuntimeValue]) -> Result<RuntimeValue> {
        let func_scope = Rc::new(RefCell::new(Scope::with_parent(self.global_scope.clone())));
        ...
        match name {
            ...
            "len" => {
                match args.first() {
                    ...
                    Some(RuntimeValue::Array(a)) | Some(RuntimeValue::List(a)) => Ok(RuntimeValue::Integer(a.len() as i64)),
            ...
            "range" => {
                match (args.get(0), args.get(1), args.get(2)) {
                    ...
                    (Some(RuntimeValue::Integer(start)), Some(RuntimeValue::Integer(end)), None) => {
                        let items: Vec<RuntimeValue> = (*start..*end).map(RuntimeValue::Integer).collect();
                        Ok(RuntimeValue::Array(items))
                    }
                    (Some(RuntimeValue::Integer(start)), Some(RuntimeValue::Integer(end)), Some(RuntimeValue::Integer(step))) => {
                        let mut items = Vec::new();
                        let mut i = *start;
                        if *step > 0 {
                            while i < *end { items.push(RuntimeValue::Integer(i)); i += step; }
                        } else if *step < 0 {
                            while i > *end { items.push(RuntimeValue::Integer(i)); i += step; }
                        }
                        Ok(RuntimeValue::Array(items))
    ...
    fn call_method(&mut self, obj: &RuntimeValue, method: &str, args: &[RuntimeValue]) -> Result<RuntimeValue> {
        match (obj, method) {
            ...
            (RuntimeValue::String(s), "contains") => {
                match args.first() {
                    Some(RuntimeValue::String(sub)) => Ok(RuntimeValue::Boolean(s.contains(sub.as_str()))),
                    _ => Err(anyhow!("contains() requires a string argument")),
                }
            }
            (RuntimeValue::String(s), "split") => {
                match args.first() {
                    Some(RuntimeValue::String(sep)) => {
                        let items: Vec<RuntimeValue> = s.split(sep.as_str())
            ...
            (RuntimeValue::String(s), "starts_with") => {
                match args.first() {
                    Some(RuntimeValue::String(prefix)) => Ok(RuntimeValue::Boolean(s.starts_with(prefix.as_str()))),
                    _ => Err(anyhow!("starts_with() requires a string argument")),
                }
            }
            (RuntimeValue::String(s), "ends_with") => {
                match args.first() {
                    Some(RuntimeValue::String(suffix)) => Ok(RuntimeValue::Boolean(s.ends_with(suffix.as_str()))),
            ...
            (RuntimeValue::Map(m), "keys") => {
                let keys: Vec<RuntimeValue> = m.keys()
            ...
            (RuntimeValue::Map(m), "contains_key") => {
                match args.first() {
            ...
            (RuntimeValue::StructInstance { type_name, .. }, _) => {
                ...
                let struct_def = self.global_scope.borrow().get(type_name)
            ...
            _ => Err(anyhow!("No method '{}' on type {}", method, obj.type_name())),
        }
    }
