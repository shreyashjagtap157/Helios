...
impl ZeroOptimizer {
    ...
    where T: Clone + Default
    {
        ...
        log::debug!("ZeRO: AllGather rank {} contributing {} elements at offset {}", 
            self.rank, chunk_size, offset);
...
impl GradientBucketeer {
    pub fn new(bucket_size_mb: usize) -> Self {
        Self {
            bucket_size_mb,
            buckets: Vec::new(),
            current_bucket: Bucket { params: Vec::new(), size_bytes: 0, data: Vec::new() },
        }
    }
    ...
    pub fn add_param(&mut self, name: &str, size_bytes: usize) {
        if self.current_bucket.size_bytes + size_bytes > self.bucket_size_mb * 1024 * 1024 {
            // Flush bucket
            let full_bucket = std::mem::replace(&mut self.current_bucket, Bucket { 
                params: Vec::new(), size_bytes: 0, data: Vec::new() 
            });
...
impl TopologyDiscovery {
    ...
    pub fn optimal_ring_order(topology: &HashMap<(usize, usize), LinkType>, gpu_count: usize) -> Vec<usize> {
