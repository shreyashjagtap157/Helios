...
fn run_omnc_emit(omnc_bin: &Path, input: &Path) -> bool {
    let out = std::env::temp_dir().join("omni_phase3_baseline.ovm");
...
#[test]
fn phase3_enrichment_baseline_stays_stable() {
    let root = repo_root();
    ...
    let cases = [
        Case {
            input_rel: "omni-lang/examples/phase3_generics_basic.omni",
            stage1_should_pass: true,
            omnc_should_pass: true,
        },
        Case {
            input_rel: "omni-lang/examples/phase3_async_await_basic.omni",
            stage1_should_pass: true,
            omnc_should_pass: true,
        },
    ];
    ...
    for case in cases {
        ...
        assert_eq!(
            ...
            "phase3 baseline mismatch for stage1 {}: expected {}, got {}",
            input.display(), case.stage1_should_pass, stage1_ok
        );
        assert_eq!(
            omnc_ok, case.omnc_should_pass,
            "phase3 baseline mismatch for omnc {}: expected {}, got {}",
