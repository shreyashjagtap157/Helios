...
impl TypeSubstitution {
    ...
    pub fn apply_to_type(&self, ty: &Type) -> Type {
        match ty {
            ...
            Type::Named(name) if self.mapping.contains_key(name) => {
            ...
            Type::Generic(name, args) => {
                if let Some(concrete) = self.mapping.get(name) {
                    ...
                    let substituted_args: Vec<Type> = args
                        .iter()
                        .map(|arg| self.apply_to_type(arg))
                        .collect();
            ...
            Type::Function(params, ret) => {
                let substituted_params: Vec<Type> = params
                    .iter()
                    .map(|p| self.apply_to_type(p))
                    .collect();
                let substituted_ret = ret.as_ref().map(|r| {
                    Box::new(self.apply_to_type(r))
                });
    ...
    pub fn transform_statement(&self, stmt: &Statement) -> Statement {
        match stmt {
            ...
            Statement::Let { name, mutable, ty, value } => {
                let substituted_type = ty.as_ref().map(|t| {
                    self.substitution.apply_to_type(t)
                });
            ...
            Statement::If { condition, then_block, else_block } => {
                ...
                let transformed_else = else_block.as_ref().map(|body| {
            ...
            Statement::Match { expr, arms } => {
                ...
                let transformed_arms = arms.iter().map(|arm| {
