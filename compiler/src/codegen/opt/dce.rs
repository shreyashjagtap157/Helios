        ...
        while worklist {
            ...
            for block in &func.blocks {
                for inst in &block.instructions {
                    match inst {
                        IrInstruction::Load { ptr, .. } => { used_vars.insert(ptr.clone()); },
                        IrInstruction::Store { ptr, value } => { 
                            used_vars.insert(ptr.clone()); 
                            if let crate::ir::IrValue::Var(v) = value { used_vars.insert(v.clone()); }
                        },
                        IrInstruction::BinOp { left, right, .. } => {
                            if let crate::ir::IrValue::Var(v) = left { used_vars.insert(v.clone()); }
                            if let crate::ir::IrValue::Var(v) = right { used_vars.insert(v.clone()); }
                        },
                        IrInstruction::Call { func: _, args, .. } => {
                             for arg in args {
                                 if let crate::ir::IrValue::Var(v) = arg { used_vars.insert(v.clone()); }
                             }
                        },
                        ...
                        IrInstruction::Select { cond, then_val, else_val, .. } => {
                            if let crate::ir::IrValue::Var(v) = cond { used_vars.insert(v.clone()); }
                            if let crate::ir::IrValue::Var(v) = then_val { used_vars.insert(v.clone()); }
                            if let crate::ir::IrValue::Var(v) = else_val { used_vars.insert(v.clone()); }
                        },
                        // ... other instructions ...
                        _ => {}
                    }
                ...
                match &block.terminator {
                    IrTerminator::Return(Some(crate::ir::IrValue::Var(v))) => { used_vars.insert(v.clone()); },
