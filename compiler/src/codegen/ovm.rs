...
use crate::ir::{IrModule, IrFunction, IrType, IrInstruction, IrTerminator, IrBinOp};
use log::{debug, info};
use std::path::Path;
...
impl OvmCodegen {
    ...
    fn emit_u16(&mut self, value: u16) {
        self.current_bytecode.extend_from_slice(&value.to_le_bytes());
    }
...
pub fn generate_ovm(ir: IrModule, output: &Path) -> Result<(), String> {
    ...
    info!("Hardware: {:?} SIMD={:?} cores={} mem={}MB gpu={}",
        hw.cpu_arch, hw.simd_level, hw.core_count,
        hw.available_memory / (1024 * 1024), hw.has_gpu);
    ...
    if let Some(ref hw) = codegen.hardware {
        let hints = format!(
            "arch={:?}\nsimd={:?}\ncores={}\nmemory={}\ngpu={}\ngc_mode={}\nthread_count={}\n",
            hw.cpu_arch, hw.simd_level, hw.core_count,
            hw.available_memory, hw.has_gpu,
            hw.optimal_gc_mode(), hw.optimal_thread_count()
        );
...
fn compile_instruction(codegen: &mut OvmCodegen, inst: &IrInstruction) -> Result<(), String> {
    ...
    let simd_available = codegen.hardware.as_ref()
    ...
    match inst {
        ...
        IrInstruction::Load { dest: _, ptr, ty: _ } => {
        ...
        IrInstruction::BinOp { dest: _, op, left: _, right: _ } => {
            let opcode = match op {
        ...
        IrInstruction::CreateClosure { dest: _, func, captures: _ } => {
            let func_idx = codegen.add_constant(OvmConstant::String(func.clone()));
        ...
        IrInstruction::CallClosure { dest: _, closure: _, args } => {
            codegen.emit(OvmOpcode::CallInd);
            codegen.emit_u8(args.len() as u8);
        }
        
        IrInstruction::AsyncSpawn { dest: _, func, args: _ } => {
            let func_idx = codegen.add_constant(OvmConstant::String(func.clone()));
            codegen.emit(OvmOpcode::LoadConst);
            codegen.emit_u32(func_idx);
        ...
        IrInstruction::TraitDispatch { dest: _, object: _, method, args } => {
        ...
        IrInstruction::VTableLookup { dest: _, object: _, trait_name: _, method_idx } => {
            codegen.emit(OvmOpcode::GetField);
            codegen.emit_u16(*method_idx as u16);
        }
        
        IrInstruction::Cast { dest: _, value: _, to_type: _ } => {
            codegen.emit(OvmOpcode::Cast);
            codegen.emit_u32(0); // Type ID placeholder
        }
        
        IrInstruction::ExtractValue { dest: _, aggregate: _, indices } => {
            for idx in indices {
                codegen.emit(OvmOpcode::GetField);
        ...
        IrInstruction::InsertValue { dest: _, aggregate: _, value: _, indices } => {
            for idx in indices {
        ...
        IrInstruction::NativeCall { dest: _, module, func, args } => {
...
fn compile_terminator(codegen: &mut OvmCodegen, term: &IrTerminator) -> Result<(), String> {
    match term {
        ...
        IrTerminator::Branch(label) => {
            let target = *codegen.label_offsets.get(label)
        ...
        IrTerminator::CondBranch { cond: _, then_label, else_label } => {
            let then_target = *codegen.label_offsets.get(then_label)
                .ok_or_else(|| format!("Unknown label: {}", then_label))?;
            let else_target = *codegen.label_offsets.get(else_label)
...
fn serialize_module(module: &OvmModule) -> Result<Vec<u8>, String> {
    ...
    bytes.extend_from_slice(&(module.symbols.len() as u32).to_le_bytes());
    // ... symbols would be written here
    let symbol_len = bytes.len() as u64 - symbol_off;
    
    // Patch offsets
    bytes[const_pool_offset_pos..const_pool_offset_pos+8].copy_from_slice(&const_pool_off.to_le_bytes());
    bytes[const_pool_offset_pos+8..const_pool_offset_pos+16].copy_from_slice(&const_pool_len.to_le_bytes());
