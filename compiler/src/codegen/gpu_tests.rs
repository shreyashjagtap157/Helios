...
mod tests {
    ...
    fn test_software_backend_basic() {
        ...
        for i in 0..size { host_data[i] = (i % 255) as u8; }
        
        backend.memcpy_h2d(ptr, &host_data).expect("H2D copy failed");
    ...
    fn test_device_manager_fallback() {
        ...
        let software_dev = devices.iter().find(|d| d.backend == GpuBackendType::Software);
        assert!(software_dev.is_some(), "Software fallback device not found");
        
        // Select it
        if let Some(pos) = devices.iter().position(|d| d.backend == GpuBackendType::Software) {
            manager.select_device(pos).unwrap();
            let ctx = manager.context().expect("Context not created for software device");
