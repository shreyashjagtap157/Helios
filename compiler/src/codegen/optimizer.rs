...
use crate::ir::{IrModule, IrFunction, IrBlock, IrInstruction, IrBinOp, IrValue, IrConst, IrTerminator};
...
impl IrOptimizer {
    pub fn new(aggressive: bool) -> Self {
        IrOptimizer {
            ...
            inline_threshold: 20,
            unroll_threshold: 50,
            opt_level: if aggressive { OptLevel::O3 } else { OptLevel::O2 },
        }
    }
    ...
    pub fn optimize(&mut self, module: &mut IrModule) -> OptimizationStats {
        info!("Starting IR optimization passes (level={:?}, aggressive={})",
    ...
    fn constant_fold_pass(&mut self, module: &mut IrModule) {
                        ...
                        IrInstruction::BinOp { dest, op, left, right } => {
                            ...
                            if let (Some(lc), Some(rc)) = (left_const, right_const) {
                                ...
                                if let Some(result) = self.eval_binop(op, &lc, &rc) {
                                    trace!("Folded constant: {} = {:?} {:?} {:?}", dest, lc, op, rc);
        ...
        debug!("Constant folding: {} expressions folded", self.stats.constant_folded);
    }

    /// Resolve an IrValue to a constant if possible
    fn resolve_constant(&self, value: &IrValue, known: &HashMap<String, IrConst>) -> Option<IrConst> {
        match value {
    ...
    fn dead_code_elimination_pass(&mut self, module: &mut IrModule) {
        for function in &mut module.functions {
            ...
            for block in &function.blocks {
                ...
                match &block.terminator {
                    IrTerminator::Return(Some(val)) => {
                        if let IrValue::Var(v) = val { used_vars.insert(v.clone()); }
                    }
                    IrTerminator::CondBranch { cond, .. } => {
                        if let IrValue::Var(v) = cond { used_vars.insert(v.clone()); }
                    }
                    _ => {}
                }
            }
            ...
            for block in &mut function.blocks {
                ...
                self.stats.instructions_removed += original_len.saturating_sub(block.instructions.len());
            }
            ...
            if function.blocks.len() > 1 {
                ...
                if removed_blocks > 0 {
                    ...
                    debug!("DCE: Removed {} unreachable blocks from {}", removed_blocks, function.name);
                }
            }
        }
        debug!("Dead code elimination: {} eliminated, {} instructions removed",
    ...
    fn collect_uses(&self, instr: &IrInstruction, used: &mut HashSet<String>) {
        match instr {
            IrInstruction::BinOp { left, right, .. } => {
                if let IrValue::Var(v) = left { used.insert(v.clone()); }
                if let IrValue::Var(v) = right { used.insert(v.clone()); }
            }
            IrInstruction::Load { ptr, .. } => { used.insert(ptr.clone()); }
            IrInstruction::Store { ptr, value } => {
                used.insert(ptr.clone());
                if let IrValue::Var(v) = value { used.insert(v.clone()); }
            }
            ...
            IrInstruction::GetField { ptr, .. } => { used.insert(ptr.clone()); }
            IrInstruction::Cast { value, .. } => {
                if let IrValue::Var(v) = value { used.insert(v.clone()); }
            }
            IrInstruction::Select { cond, then_val, else_val, .. } => {
                if let IrValue::Var(v) = cond { used.insert(v.clone()); }
                if let IrValue::Var(v) = then_val { used.insert(v.clone()); }
                if let IrValue::Var(v) = else_val { used.insert(v.clone()); }
            }
            ...
            IrInstruction::CreateClosure { captures, .. } => {
                for cap in captures { used.insert(cap.clone()); }
            }
            ...
            IrInstruction::AsyncAwait { future, .. } => { used.insert(future.clone()); }
            ...
            IrInstruction::VTableLookup { object, .. } => { used.insert(object.clone()); }
            IrInstruction::ExtractValue { aggregate, .. } => { used.insert(aggregate.clone()); }
            ...
            while merged {
                ...
                while i < function.blocks.len() {
                    ...
                    if let IrTerminator::Branch(target) = &function.blocks[i].terminator {
                        ...
                        if let Some(target_idx) = function.blocks.iter().position(|b| b.label == target) {
                            ...
                            if pred_count <= 1 && target_idx != i && target_idx < function.blocks.len() {
                                ...
                                if current_idx < function.blocks.len() {
                                    function.blocks[current_idx].instructions
                                        .extend(target_block.instructions);
                                    function.blocks[current_idx].terminator = target_block.terminator;
                                    self.stats.blocks_merged += 1;
                                    merged = true;
                                    trace!("Merged block '{}' into '{}'", target, function.blocks[current_idx].label);
                                }
    ...
    fn build_predecessor_map(&self, function: &IrFunction) -> HashMap<String, Vec<String>> {
        ...
        for block in &function.blocks {
            match &block.terminator {
                IrTerminator::Branch(target) => {
                    preds.entry(target.clone()).or_default().push(block.label.clone());
                }
                IrTerminator::CondBranch { then_label, else_label, .. } => {
                    preds.entry(then_label.clone()).or_default().push(block.label.clone());
                    preds.entry(else_label.clone()).or_default().push(block.label.clone());
                }
                IrTerminator::Return(_) | IrTerminator::Unreachable => {}
            }
        }
    ...
    fn tail_call_optimization_pass(&mut self, module: &mut IrModule) {
        for function in &mut module.functions {
            ...
            let entry_label = function.blocks.first()
                .map(|b| b.label.clone())
                .unwrap_or_default();
            ...
            for block in &mut function.blocks {
                ...
                if let Some(last_inst) = block.instructions.last() {
                    if let IrInstruction::Call { dest: Some(dest), func, args: _call_args } = last_inst {
                        ...
                        if let IrTerminator::Return(Some(IrValue::Var(ret_var))) = &block.terminator {
                            if ret_var == dest {
                                ...
                                if func == &func_name {
                                    ...
                                    debug!("TCO: Converting recursive tail call in {} to jump", func_name);
        ...
        debug!("Tail call optimization: {} tail calls optimized", self.stats.tail_calls_optimized);
    }
    ...
    fn function_inlining_pass(&mut self, module: &mut IrModule) {
        ...
        for function in &module.functions {
            ...
            if total_instructions <= self.inline_threshold && total_instructions > 0 {
                ...
                let is_recursive = function.blocks.iter()
                    .flat_map(|b| &b.instructions)
                    .any(|i| matches!(i, IrInstruction::Call { func, .. } if func == &function.name));
        ...
        debug!("Inlining: {} candidate functions: {:?}",
               inline_bodies.len(), inline_bodies.keys().collect::<Vec<_>>());
                        ...
                        if let Some(callee) = inline_bodies.get(func) {
                            ...
                            for (bidx, callee_block) in callee.blocks.iter().enumerate() {
                                ...
                                for ci in &callee_block.instructions {
                                    renamed_insts.push(self.rename_instruction(ci, &prefix, &var_map));
                                }
        ...
        debug!("Function inlining: {} call sites inlined", self.stats.functions_inlined);
    }

    /// Rename an instruction's variables with a prefix (for inlining)
    fn rename_instruction(&self, inst: &IrInstruction, prefix: &str, var_map: &HashMap<String, IrValue>) -> IrInstruction {
        match inst {
            IrInstruction::BinOp { dest, op, left, right } => {
                IrInstruction::BinOp {
            ...
            IrInstruction::Load { dest, ptr, ty } => {
                IrInstruction::Load {
                    dest: format!("{}_{}", prefix, dest),
                    ptr: var_map.get(ptr)
                        .and_then(|v| if let IrValue::Var(n) = v { Some(n.clone()) } else { None })
            ...
            IrInstruction::Call { dest, func, args } => {
                IrInstruction::Call {
                    ...
                    func: func.clone(),
                    args: args.iter().map(|a| self.rename_value(a, prefix, var_map)).collect(),
                }
            }
            IrInstruction::Phi { dest, ty, incoming } => {
                IrInstruction::Phi {
                    dest: format!("{}_{}", prefix, dest),
                    ty: ty.clone(),
                    incoming: incoming.iter().map(|(v, l)| (self.rename_var_name(v, prefix, var_map), format!("{}_{}", prefix, l))).collect(),
                }
            }
            ...
            IrInstruction::CreateClosure { dest, func, captures } => {
                IrInstruction::CreateClosure {
                    dest: format!("{}_{}", prefix, dest),
                    func: func.clone(),
                    captures: captures.iter().map(|c| self.rename_var_name(c, prefix, var_map)).collect(),
                }
            }
    ...
    fn rename_value(&self, value: &IrValue, prefix: &str, var_map: &HashMap<String, IrValue>) -> IrValue {
        match value {
            IrValue::Var(name) => {
    ...
    fn rename_var_name(&self, name: &str, prefix: &str, var_map: &HashMap<String, IrValue>) -> String {
        var_map.get(name)
            .and_then(|v| if let IrValue::Var(n) = v { Some(n.clone()) } else { None })
    ...
    fn cse_pass(&mut self, module: &mut IrModule) {
                ...
                for instr in &block.instructions {
                    match instr {
                        IrInstruction::BinOp { dest, op, left, right } => {
                            ...
                            if let Some(existing_var) = expr_to_var.get(&expr_key)
        ...
        debug!("CSE: {} redundant computations eliminated", self.stats.cse_eliminated);
    }
    ...
    fn strength_reduction_pass(&mut self, module: &mut IrModule) {
        for function in &mut module.functions {
            for block in &mut function.blocks {
                ...
                for instr in &block.instructions {
                    match instr {
                        IrInstruction::BinOp { dest, op: IrBinOp::Mul, left, right } => {
                        ...
                        IrInstruction::BinOp { dest, op: IrBinOp::Div, left, right } => {
                        ...
                        IrInstruction::BinOp { dest, op: IrBinOp::Mod, left, right } => {
        ...
        debug!("Strength reduction: {} operations reduced", self.stats.strength_reduced);
    }
    ...
    fn copy_propagation_pass(&mut self, module: &mut IrModule) {
        for function in &mut module.functions {
            ...
            for block in &mut function.blocks {
                for instr in &block.instructions {
                    // Detect copies: x = y + 0
                    if let IrInstruction::BinOp { dest, op: IrBinOp::Add, left, right } = instr {
        ...
        debug!("Copy propagation: {} copies propagated", self.stats.copies_propagated);
    }

    /// Propagate copies within an instruction's operands
    fn propagate_copies_in_instruction(&self, instr: &mut IrInstruction, copies: &HashMap<String, IrValue>) -> usize {
        let mut count = 0;
    ...
    fn algebraic_simplification_pass(&mut self, module: &mut IrModule) {
        ...
        debug!("Algebraic simplification: {} simplifications", self.stats.algebraic_simplified);
    }
    ...
    fn licm_pass(&mut self, module: &mut IrModule) {
        for function in &mut module.functions {
            ...
            let block_indices: HashMap<String, usize> = function.blocks.iter()
                .enumerate()
            ...
            for (header_idx, latch_idx) in &loops {
                ...
                if *header_idx > 0 {
                    ...
                    for block_idx in *header_idx..=*latch_idx {
                        if block_idx >= function.blocks.len() { continue; }
                        ...
                        if !to_hoist.is_empty() {
                            ...
                            for hoisted in to_hoist {
                                trace!("LICM: Hoisting {:?} from block {} to preheader {}",
                                       hoisted,
        ...
        debug!("LICM: {} instructions hoisted out of loops", self.stats.licm_hoisted);
    }
    ...
    fn loop_unrolling_pass(&mut self, module: &mut IrModule) {
        ...
        for function in &mut module.functions {
            ...
            for (i, block) in function.blocks.iter().enumerate() {
                if let IrTerminator::CondBranch { then_label, else_label, .. } = &block.terminator {
            ...
            for (block_idx, factor) in to_unroll.into_iter().rev() {
                ...
                trace!("Unrolled loop at block {} by factor {}", function.blocks[block_idx].label, factor);
            }
        }
        debug!("Loop unrolling: {} loops unrolled", self.stats.loops_unrolled);
    }
...
mod tests {
    ...
    fn make_test_module() -> IrModule {
        IrModule {
            ...
            functions: vec![IrFunction {
                name: "add".to_string(),
                params: vec![("x".to_string(), IrType::I64), ("y".to_string(), IrType::I64)],
    ...
    fn test_strength_reduction_power_of_2() {
        ...
        assert_eq!(opt.is_power_of_2_const(&IrValue::Const(IrConst::Int(8))), Some(3));
        assert_eq!(opt.is_power_of_2_const(&IrValue::Const(IrConst::Int(16))), Some(4));
        assert_eq!(opt.is_power_of_2_const(&IrValue::Const(IrConst::Int(7))), None);
        assert_eq!(opt.is_power_of_2_const(&IrValue::Const(IrConst::Int(1))), Some(0));
    }
    ...
    fn test_algebraic_simplification() {
        ...
        module.functions[0].blocks[0].instructions.insert(0, IrInstruction::BinOp {
    ...
    fn test_cse_eliminates_duplicates() {
        ...
        assert!(opt.stats.cse_eliminated > 0, "CSE should eliminate duplicate computation");
    }
    ...
    fn test_block_merging() {
        ...
        assert!(module.functions[0].blocks.len() < original_blocks, "Blocks should be merged");
